...
sitka<-getParms(weather=clm_df_full,
                ...
                E_S1 =100, #Cumulitive evap threshold (kg^m-2) - sensitive to length of time-step, e.g. monthly time-step means wetting event only occurs at end of month
                E_S2 =500, #how quickly evaporation rate declines with accumulated phase 2 evaporation - based on soil structure
                MaxASW_state=50,
                K_drain=0.1,
...
#codM[nm[i]]<-sitka[nm[i]]#<-codM[nm[i]]
...
codM<-getSample(out2, start = 100, coda = TRUE, thin = 1)
#bayesplot::mcmc_trace(codM)
#get some previous run parameter estimates#
codM<-mergeChains(out2$chain)
