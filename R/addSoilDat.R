...
#soilLocVals<-raster("C:\\Users\\aaron.morris\\OneDrive - Forest Research\\Documents\\Projects\\PRAFOR\\models\\spatial_soil_data\\soil_depth.tif")
...
#clmRast$soil_depth<-getValues(mergedSoil$soil_depth)
...
#clmRast2<-dplyr::filter(clmRast, lat <=max(simDat$lat))
#clmRast2<-dplyr::filter(clmRast2, lat >=min(simDat$lat))
#clmRast2<-dplyr::filter(clmRast2, lon >=min(simDat$lon))
#clmRast2<-dplyr::filter(clmRast2, lon <=max(simDat$lon))

#ggplot() +
#  geom_raster(data = clmRast2 , aes(x = lon, y = lat, fill = vals ))+
#  theme_bw()+
#  geom_point(aes(x=simDat$lon,y=simDat$lat),col="black",alpha=0.1)+
